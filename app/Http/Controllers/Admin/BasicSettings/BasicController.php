...
class BasicController extends Controller
{
    ...
    public function updateGoogleMapApi(Request $request)
    {
        $rules = [
            ...
            'google_maps_api_key' => 'required_if:google_map_api_key_status,1',
        ];

        $messages = [
            'google_maps_api_key.required_if' => 'The Google Maps API Key field is required when status is Active.'
        ];
    ...
    public function updatePixel(Request $request)
    {
        $rules = [
            ...
            'pixel_id' => 'required_if:pixel_status,1',
        ];
        ...
        Session::flash('success', __('Meta (Facebook) Pixel updated successfully') . '!');
        return redirect()->back();
    }
    ...
    {
        $rules = [
            'google_analytics_status' => 'required',
            'google_analytics_id' => 'required_if:google_analytics_status,1',
        ];
        ...
        Session::flash('success', __('Google Analytics updated successfully') . '!');
        return redirect()->back();
    }

    public function updateGoogleGemini(Request $request)
    {
        $rules = [
            'google_gemini_status' => 'required',
            'google_gemini_api_key' => 'required_if:google_gemini_status,1',
        ];
        ...
        DB::table('basic_settings')->updateOrInsert(
            ['uniqid' => 12345],
            [
                'google_gemini_status' => $request->google_gemini_status,
                'google_gemini_api_key' => $request->google_gemini_api_key,
                'google_gemini_prompt' => $request->google_gemini_prompt,
            ]
        );
        ...
        Session::flash('success', __('Google Gemini AI Engine updated successfully') . '!');
        return redirect()->back();
    }
    ...
    public function updateWhatsApp(Request $request)
    {
        ...
        // Auto update .env with WHATSAPP_API variables
        $array = [];
        if ($request->has('whatsapp_api_url')) {
            $array['WA_API_URL'] = $request->whatsapp_api_url ?? '';
        }
        if ($request->has('whatsapp_api_key')) {
            $array['WA_API_KEY'] = $request->whatsapp_api_key ?? '';
        }
    ...
    public function hotelTaxAmount()
    {
        ...
        return view('admin.perahu-management.tax', ['data' => $data]);
    }
