...
class CityController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('admin.lokasi-management.location.city.index', $information);
    }
    ...
    public function destroy($id)
    {
        ...
        if (count($contents) > 0) {
            return redirect()->back()->with('warning',  __('First delete all the lokasi of this City') . '!');
        } else {
            @unlink(public_path('assets/img/location/city/') . $city->feature_image);
            $city->delete();
    ...
    public function bulkDestroy(Request $request)
    {
        ...
        if ($errorOccurred == true) {
            Session::flash('warning', __('First delete all the lokasi of these City') . '!');
        } else {
