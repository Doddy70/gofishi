...
class PerahuController extends Controller
{
    public function settings()
    {
        ...
        return view('admin.perahu-management.settings', ['info' => $info]);
    }
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.perahu-management.index', $information);
    }
    ...
    public function updateFeatured(Request $request)
    {
        ...
        $vendor_id = Perahu::where('id', $request->room_id)->pluck('vendor_id')->first();
        ...
        Session::flash('success', __('Perahu Featured successfully') . '!');
        return  redirect()->back();
    }
    ...
    public function selectVendor()
    {
        ...
        $information['vendors'] = Vendor::join('memberships', 'vendors.id', '=', 'memberships.vendor_id')
            ...
            ->get();
        return view('admin.perahu-management.select-vendor', $information);
    }
    public function findVendor(Request $request)
    {
        return redirect()->route('admin.perahu_management.create_perahu', ['vendor_id' => $request->vendor_id ?? 0]);
    }
    ...
    public function create($id)
    {
        ...
        return view('admin.perahu-management.create', $information);
    }
    public function imagesstore(Request $request)
    {
        ...
        $directory = public_path('assets/img/perahu/room-gallery/');
    ...
    public function imagermv(Request $request)
    {
        ...
        @unlink(public_path('assets/img/perahu/room-gallery/') . $pi->image);
        $pi->delete();
        return $pi->id;
    }
    ...
    public function store(RoomStoreRequest $request)
    {
        ...
        if ($totalHotelAdded < $current_packageRoom) {
            ...
            if ($request->feature_image) {
                ...
                $featuredDir = public_path('assets/img/perahu/featureImage/');
            ...
            $in['video_url'] = $request->video_url;

            $room = Perahu::create($in);
            ...
            if (!empty(array_filter($prices))) {
                ...
                for ($i = 0; $i < $hours->count(); $i++) {
                    ...
                    $hourlyRoomPrice->price = $prices[$i] ?? null;

                    $hourlyRoomPrice->save();
                }
            }
            $roomprice = Perahu::findOrFail($room->id);
    ...
    {
        $information['room'] =  Perahu::findorfail($id);
        ...
        return view('admin.perahu-management.additional-service', $information);
    }
    ...
    {
        $vendorId = Perahu::where('id', $id)->pluck('vendor_id')->first();
        ...
        $information['room'] = Perahu::with('room_galleries')->findOrFail($id);
        $information['languages'] = Language::all();
        $information['defaultLang'] = Language::query()->where('is_default', 1)->first();
        $information['bookingHours'] = BookingHour::orderBy('serial_number', 'asc')->get();
        $information['prices'] = HourlyRoomPrice::Where('room_id', $id)->get();
        
        // Admin can always edit regardless of vendor membership status
        return view('admin.perahu-management.edit', $information);
    }
    ...
    public function update(RoomUpdateRequest $request, $id)
    {
        ...
        if ($request->hasFile('thumbnail')) {
            ...
            copy($featuredImgURL, $featuredDir . $featuredImgName);
            @unlink(public_path('assets/img/perahu/featureImage/') . $room->feature_image);
