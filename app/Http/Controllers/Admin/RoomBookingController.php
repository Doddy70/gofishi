...
class RoomBookingController extends Controller
{
    public function __construct(
        protected \App\Actions\Booking\UpdateBookingStatus $updateBookingStatus,
        protected \App\Services\NotificationService $notificationService
    ) {
        $this->updateBookingStatus = $updateBookingStatus;
        $this->notificationService = $notificationService;

        // Tier 3 (Staff) can view reports and lists
        $this->middleware('admin.tier:3')->only(['index', 'report', 'details']);
        
        // Tier 2 (Manager) can change status
        $this->middleware('admin.tier:2')->only(['updatePaymentStatus', 'updateOrderStatus']);
        
        // Tier 1 (Super) can delete or export
        $this->middleware('admin.tier:1')->only(['delete', 'bulkDelete', 'exportReport']);
    }
    public function index(Request $request)
    {
        ...
        if (URL::current() == Route::is('admin.perahu_bookings.all_bookings')) {
            $information['bookings'] = Booking::when($order_number, function ($query, $order_number) {
                ...
                ->orderBy('id', 'desc')
                ->paginate(10);
        } else if (URL::current() == Route::is('admin.perahu_bookings.paid_bookings')) {
            $information['bookings'] = Booking::when($order_number, function ($query, $order_number) {
                return $query->where('order_number', 'like', '%' . $order_number . '%');
            })
                ->when($title, function ($query) use ($roomIds) {
                ...
                ->orderBy('id', 'desc')
                ->paginate(10);
        } else if (URL::current() == Route::is('admin.perahu_bookings.unpaid_bookings')) {
            $information['bookings'] = Booking::when($order_number, function ($query, $order_number) {
                return $query->where('order_number', 'like', '%' . $order_number . '%');
            })
                ->when($title, function ($query) use ($roomIds) {
        ...
        return view('admin.perahu-booking.bookings', $information);
    }
    public function details(Request $request, $id)
    {
        ...
        return view('admin.perahu-booking.details', $information);
    }
    ...
    public function updatePaymentStatus(Request $request)
    {
        ...
        $old_status = $bookingInfo->payment_status;
        ...
        if ($old_status != $request->payment_status) {
            $roomInfo = $bookingInfo->hotelRoom()->first();
            $roomContent = $roomInfo ? $roomInfo->room_content()->first() : null;

            $notifData = [
                'username' => $bookingInfo->booking_name,
                'perahu_name' => $roomContent ? $roomContent->title : 'Perahu',
                'booking_date' => $bookingInfo->check_in_date,
                'order_id' => $bookingInfo->order_number,
                'recipient' => $bookingInfo->booking_email
            ];

            if ($request->payment_status == '1') {
                $this->notificationService->send($bookingInfo, 'payment_received', $notifData);
            }
        }
    ...
    public function bookedDates(Request $request)
    {
        ...
        return response()->json([
            'success' => route('admin.perahu_bookings.booking_form', ['room_id' => $roomId])
        ]);
    }
    public function bookingForm(Request $request)
    {
        ...
        return view('admin.perahu-booking.booking-form', $information);
    }
    ...
    public function editBookingDetails(Request $request, $id)
    {
        ...
        return view('admin.perahu-booking.edit-booking', $information);
    }
    ...
    public function report(Request $request)
    {
        ...
        if ($from && $to) {
            $records = Booking::query()
                ...
                ->when($paymentGateway, function ($query, $paymentGateway) {
            ...
            // Add Stats
            $information['total_bookings'] = $records->count();
            $information['total_amount'] = $records->where('payment_status', 1)->sum('grand_total');
        } else {
            Session::put('product_orders', null);
            $information['orders'] = [];
            $information['total_bookings'] = 0;
            $information['total_amount'] = 0;
        }

        return view('admin.perahu-booking.report', $information);
    }
