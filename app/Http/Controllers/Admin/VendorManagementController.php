...
class VendorManagementController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $rules = [
            ...
            'boat_ownership_file' => 'required|mimes:jpg,jpeg,png,pdf|max:2048',
            'driving_license_file' => 'required|mimes:jpg,jpeg,png,pdf|max:2048',
            'self_photo_file' => 'required|mimes:jpg,jpeg,png,pdf|max:2048',
        ];
        ...
        if ($defaultLang) {
          ...
          $fullName = $request->input($code . '_name');
          $nameParts = explode(' ', $fullName, 2);
          
          $in['fname'] = $nameParts[0] ?? $request->username;
          $in['lname'] = $nameParts[1] ?? '';
          $in['lang_code'] = 'admin_ ' . $defaultLang->code;
          $in['code'] = $defaultLang->code;
        }
        ...
        foreach ($languages as $lang) {
            ...
            if (
            $lang->is_default == 1 ||
            $request->filled($lang->code . '_name')
            ) {
                $vendorInfo->language_id = $lang->id;
                $vendorInfo->vendor_id = $vendor_id;
                $vendorInfo->name = $request[$lang->code . '_name'];
                $vendorInfo->country = $request[$lang->code . '_country'];
                $vendorInfo->city = $request[$lang->code . '_city'];
                $vendorInfo->state = $request[$lang->code . '_state'];
                $vendorInfo->zip_code = $request[$lang->code . '_zip_code'];
                $vendorInfo->address = $request[$lang->code . '_address'];
                $vendorInfo->details = $request[$lang->code . '_details'];
                $vendorInfo->save();
            }
        }

        Session::flash('success', __('New vendor added successfully!'));
        return Response::json(['status' => 'success', 'link' => route('admin.vendor_management.registered_vendor')], 200);
    }
    ...
    public function updateDocumentStatus(Request $request)
    {
        ...
        $status = $request->document_verified;
        $rejectionReason = $request->rejection_reason;

        $vendor->update([
            'document_verified' => $status,
            'rejection_reason' => ($status == 2) ? $rejectionReason : null
        ]);
        ...
        if ($status == 1) {
            $data['templateType'] = 'document_verification_approved';
            $mailer->mailFromAdmin($data);
        } elseif ($status == 2) {
            $data['templateType'] = 'document_verification_rejected';
            $data['rejection_reason'] = $rejectionReason;
            $mailer->mailFromAdmin($data);
        }

        Session::flash('success', __('Document verification status updated successfully') . '!');
        return redirect()->back();
    }
    ...
    public function update(Request $request, $id, Vendor $vendor)
    {
        ...
        foreach ($documentFields as $docField) {
            if ($request->hasFile($docField)) {
                ...
                $fileName = uniqid() . '-' . $docField . '.' . $file->getClientOriginalExtension();
                
                // Store in private storage
                $file->storeAs('private/vendor-documents', $fileName);

                // Delete old file if exists in private storage
                if (!empty($vendor->$docField)) {
                    \Illuminate\Support\Facades\Storage::delete('private/vendor-documents/' . $vendor->$docField);
                }

                $in[$docField] = $fileName;
            }
        }
