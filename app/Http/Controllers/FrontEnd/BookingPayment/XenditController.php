...
class XenditController extends Controller
{
    ...
    public function callback(Request $request)
    {
        ...
        $externalId = $data['external_id'] ?? null; // expected to match bookings.order_number
        $xenditInvoiceId = $data['id'] ?? null;

        if ($status === 'PAID' && !empty($externalId)) {
            $booking = Booking::where('payment_status', '!=', 1)
                              ->where('order_number', $externalId) // Asumsi external_id == order_number
                              ->first();

            if ($booking) {
                try {
                    $this->processPaidBooking->execute($booking, $xenditInvoiceId);
                } catch (\Exception $e) {
                    Log::error('[Xendit Webhook] processPaidBooking failed: ' . $e->getMessage());
