...
class HomeController extends Controller
{
  public function index(Request $request)
  {
    ...
    // Pastikan data Dermaga (Hubs) dari tabel Hotel juga tersedia untuk section 'Destinations'
    $information['hubs'] = \App\Models\Hotel::join('hotel_contents', 'hotels.id', '=', 'hotel_contents.hotel_id')
        ->where('hotel_contents.language_id', $language->id)
        ->where('hotels.status', '1')
        ->select('hotels.*', 'hotel_contents.title as name')
        ->get();
