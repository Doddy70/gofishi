...
class MidtransBankController extends Controller
{
    public function bankNotify(Request $request)
    {
        if (session()->has('userType') && session()->get('userType') == 'vendor') {
            ...
            if ($paymentFor == "membership") {
                ...
                $amount = $requestData['price'];
                $password = Carbon::now()->format('ymdHis') . Str::upper(Str::random(4));
                $checkout = new VendorCheckoutController();
                $vendor = $checkout->store($requestData, $transaction_id, $transaction_details, $amount, $bs, $password);
            ...
            $order->order_number = Carbon::now()->format('ymdHis') . Str::upper(Str::random(4));
    ...
    public function cancelPayment()
    {
        ...
        if ($userType == 'vendor') {
            ...
            return redirect()->route('frontend.perahu.booking.cancel')->with(['alert-type' => 'error', 'message' => 'Payment Canceled.']);
        }
    }
}
