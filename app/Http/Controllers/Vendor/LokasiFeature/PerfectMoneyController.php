...
class PerfectMoneyController extends Controller
{
    ...
    public static function index(Request $request, $event_id)
    {
        ...
        $val['STATUS_URL'] = route('vendor.lokasi_management.lokasi.purchase_feature.perfect_money.notify');
        $val['PAYMENT_URL'] = route('vendor.lokasi_management.lokasi.purchase_feature.perfect_money.notify');
        $val['PAYMENT_URL_METHOD'] = 'GET';
        $val['NOPAYMENT_URL'] = route('frontend.perahu.booking.cancel');
    ...
    public function notify(Request $request)
    {
        ...
        if ($request->PAYEE_ACCOUNT == $perfectMoneyInfo['perfect_money_wallet_id'] && $unit == $currencyInfo->base_currency_text && $track == $id && $amo == round($final_amount, 2)) {
            ...
            $bs = Basic::first();
            $hotelFeature = new LokasiFeatureController();
