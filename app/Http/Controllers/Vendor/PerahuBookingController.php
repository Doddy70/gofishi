...
class PerahuBookingController extends Controller
{
    protected $notificationService;
    protected $updateBookingStatus;

    public function __construct(NotificationService $notificationService, \App\Actions\Booking\UpdateBookingStatus $updateBookingStatus)
    {
    ...
    public function index(Request $request)
    {
        ...
        if (URL::current() == Route::is('vendor.perahu_bookings.all_bookings')) {
            $information['bookings'] = Booking::Where('vendor_id', $vendor_id)
                ...
                ->orderBy('id', 'desc')
                ->paginate(10);
        } else if (URL::current() == Route::is('vendor.perahu_bookings.paid_bookings')) {
            $information['bookings'] = Booking::Where('vendor_id', $vendor_id)
                ->when($order_number, function ($query, $order_number) {
                    return $query->where('order_number', 'like', '%' . $order_number . '%');
                })
                ...
                ->orderBy('id', 'desc')
                ->paginate(10);
        } else if (URL::current() == Route::is('vendor.perahu_bookings.unpaid_bookings')) {
            $information['bookings'] = Booking::Where('vendor_id', $vendor_id)
                ->when($order_number, function ($query, $order_number) {
                    return $query->where('order_number', 'like', '%' . $order_number . '%');
                })
        ...
        return view('vendors.perahu-booking.bookings', $information);
    }
    public function details(Request $request, $id)
    {
        ...
        return view('vendors.perahu-booking.details', $information);
    }
    ...
    public function updateOrderStatus(Request $request)
    {
        ...
        $result = $this->updateBookingStatus->execute(
            $booking, 
    ...
    public function bookedDates(Request $request)
    {
        ...
        return response()->json([
            'success' => route('vendor.perahu_bookings.booking_form', ['room_id' => $roomId])
        ]);
    }
    public function bookingForm(Request $request)
    {
        ...
        if ($vendorTotalBooking >= $vendorTotalBookingInPackage) {
            ...
            return redirect()->route('vendor.perahu_bookings.all_bookings', ['language' => Auth::guard('vendor')->user()->code]);
        }
        ...
        if ($permissions) {
            ...
            return view('vendors.perahu-booking.booking-form', $information);
        } else {
    ...
    public function makeBooking(AdminRoomBookingRequest $request)
    {
        ...
        $bookingInfo = Booking::query()->create([
            'order_number' => Carbon::now()->format('ymdHis') . Str::upper(Str::random(4)),
            'user_id' => null,
        ...
        Session::flash('success', __('Perahu has booked successfully') . '!');
    ...
    public function getPrice(Request $request, $slug, $id)
    {
        ...
        return view('vendors.perahu-booking.room-price', $information)->render();
    }
    public function editBookingDetails(Request $request, $id)
    {
        ...
        if (!$room) {
            Session::flash('warning', __('Perahu has been deleted') . '!');
            return redirect()->route('admin.perahu_bookings.all_bookings', ['language' => 'en']);
        }
        ...
        return view('vendors.perahu-booking.edit-booking', $information);
    }
    ...
    public function updateBooking(AdminRoomBookingRequest $request)
    {
        ...
        if ($old_status != $request->payment_status) {
            try {
                // Notification Data
                ...
                if ($request->payment_status == '1') {
                    $this->notificationService->send($booking, 'payment_received', $notifData);
    ...
    public function getPriceForEdit(Request $request, $slug, $id)
    {
        ...
        return view('admin.perahu-booking.room-price', $information)->render();
    }
    ...
    public function report(Request $request)
    {
        ...
        $query = Booking::query()
            ...
            ->when($from, function ($query, $from) {
                return $query->whereDate('created_at', '>=', Carbon::parse($from));
            })
            ->when($to, function ($query, $to) {
                return $query->whereDate('created_at', '<=', Carbon::parse($to));
            })
        ...
        $information['total_revenue'] = $statsQuery->where('payment_status', 1)->sum('grand_total');
        $information['total_bookings'] = $statsQuery->count();
        $information['confirmed_bookings'] = $statsQuery->where('order_status', 'approved')->count();

        $records = $query->orderByDesc('id');
        Session::put('product_orders', $records->get());
        $information['orders'] = $records->paginate(10);

        return view('vendors.perahu-booking.report', $information);
    }
    ...
    public function storeUserReview(Request $request)
    {
        ...
        \App\Models\UserReview::updateOrCreate(
            ['booking_id' => $request->booking_id, 'vendor_id' => $vendor_id, 'user_id' => $request->user_id],
            ['rating' => $request->rating, 'review_text' => $request->review_text]
        );

        Session::flash('success', __('Berhasil mengirim ulasan!'));
        return redirect()->back();
    }
}
