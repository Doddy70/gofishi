...
namespace App\Http\Controllers\Vendor\PerahuFeature;
use Illuminate\Support\Facades\Validator;

use App\Http\Controllers\Controller;
use App\Http\Controllers\Vendor\PerahuFeature\PayPalController;
use App\Http\Controllers\Vendor\PerahuFeature\OfflineController;
...
class PerahuFeatureController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request['gateway'] == 'paypal') {
            ...
            $cancel_url = route('vendor.perahu_management.perahu.purchase_feature.iyzico.cancle');
            ...
            return $perfect_money->index($request, 'perahu featured');
        } else if ($request['gateway'] == 'phonepe') {

            $phonepe = new PhonepeController();
            return $phonepe->index($request, 'perahu featured');
        } else if ($request['gateway']  == 'xendit') {

            $xendit = new XenditController();
            return $xendit->index($request, 'perahu feature');
        ...
        if ($be->smtp_status == 1) {
            ...
            try {
                $data = [
                    ...
                    'subject' => $subject,
                    'body' => $body,
                    'invoice' => public_path('assets/file/invoices/perahu-feature/' . $invoice)
                ];
                if ($be->smtp_status == 1) {
...
Thank you for your attention to this matter.";
    ...
    public function generateInvoice($order)
    {
        ...
        $directory = public_path('assets/file/invoices/perahu-feature/');
        ...
        Pdf::loadView('frontend.pdf.perahu-feature', compact('order', 'position', 'currency'))->save($fileLocated);
