...
class VendorController extends Controller
{
    ...
    public function signup()
    {
        if (Auth::guard('vendor')->check()) {
            ...
            $information['bs'] = Basic::query()->select('facebook_login_status', 'google_login_status')->first();
    ...
    public function signupSuccess()
    {
        ...
        $information['pageHeading'] = $misc->getPageHeading($language);
        $information['bgImg'] = $misc->getBreadcrumb();
        $information['setting'] = DB::table('basic_settings')->where('uniqid', 12345)->select('vendor_email_verification', 'vendor_admin_approval')->first();
        
        return view('frontend.vendor.auth.signup-success', $information);
    }
    ...
    public function create(RegistrationRequest $request)
    {
        ...
        return redirect()->route('vendor.signup_success');
    }
    ...
    public function confirm_email()
    {
        ...
        if (!$token) {
            Session::flash('error', __('Invalid verification token') . '!');
            return redirect()->route('vendor.login');
        }

        $user = Vendor::where('verification_token', $token)->first();
        if (!$user) {
            Session::flash('error', __('Invalid or expired verification link.') . '!');
            return redirect()->route('vendor.login');
        }
    ...
    public function dashboard()
    {
        ...
        // Monthly Revenue Stats for Charts
        $revenues = \App\Models\Booking::query()->select(DB::raw('month(created_at) as month'), DB::raw('sum(grand_total) as total'))
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            foreach ($revenues as $rev) {
                if ($rev->month == $i) {
                    $revenueFound = true;
                    array_push($revenueArr, $rev->total);
                    break;
                }
            }
            if (!$revenueFound) array_push($revenueArr, 0);
        ...
        $information['revenueArr'] = $revenueArr;
        $information['visitorArr'] = $visitorArr;

        // Fetch 5 Most Recent Bookings for Dashboard Action Widget
        $information['recent_bookings'] = \App\Models\Booking::with(['user', 'hotelRoom', 'hotel'])
            ->where('vendor_id', $vendor_id)
            ->orderBy('id', 'desc')
            ->take(5)
            ->get();
    ...
    public function changeTheme(Request $request)
    {
        ...
        DB::table('vendors')->where('id', $vendor->id)->update([
            'vendor_theme_version' => $request->vendor_theme_version
        ]);
        ...
        foreach ($languages as $language) {
            ...
            VendorInfo::updateOrCreate(
                ...
                [
                    ...
                    'occupation' => $request[$code . '_occupation'],
                    'languages' => $request[$code . '_languages'],
                ]
            );
        }
    ...
    public function verifyIdentityWizard()
    {
        $vendor = Auth::guard('vendor')->user();
        $information['vendor'] = Vendor::find($vendor->id);
        
        return view('frontend.vendor.auth.verification_wizard', $information);
    }

    public function submitVerifyIdentity(Request $request)
    {
        $vendor = Auth::guard('vendor')->user();
        ...
        $validator = Validator::make($request->all(), $rules);
        if ($validator->fails()) {
            Session::flash('error', __('Pastikan nama Anda sudah diisi dengan benar.'));
        ...
        $in = [
            'fname' => $request->fname,
            'lname' => $request->lname,
            'document_verified' => 0, // DOC_PENDING
        ];

        $documentFields = ['ktp_file', 'driving_license_file', 'self_photo_file'];
        foreach ($documentFields as $field) {
            if ($request->hasFile($field)) {
                $file = $request->file($field);
                $fileName = uniqid() . '-' . $field . '.' . $file->getClientOriginalExtension();
        ...
        $vModel = Vendor::findOrFail($vendor->id);
        $vModel->update($in);

        Session::flash('success', __('Dokumen verifikasi identitas Anda telah berhasil dikirim dan masuk antrean tinjauan tim kami.'));
        return redirect()->route('vendor.dashboard');
    }
