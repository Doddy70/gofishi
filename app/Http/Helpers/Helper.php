...
if (!function_exists('symbolPrice')) {
  function symbolPrice($price)
  {
    $basic = Basic::where('uniqid', 12345)
      ->select('base_currency_symbol_position', 'base_currency_symbol', 'base_currency_text')
      ->first();

    $symbol = $basic->base_currency_symbol;
    $position = $basic->base_currency_symbol_position;
    $currencyText = strtoupper((string)$basic->base_currency_text);

    $isIdr = $currencyText === 'IDR' || $symbol === 'Rp' || $symbol === 'Rp.' || str_contains(strtoupper($symbol), 'RP');
    // For IDR, we use dot for thousands and comma for decimal with 2 digits
    $formatted = $isIdr ? number_format((float)$price, 2, ',', '.') : number_format((float)$price, 2, '.', ',');

    if ($position == 'left') {
      $data = $symbol . ($isIdr ? ' ' : '') . $formatted;
      return str_replace('  ', ' ', trim($data));
    }

    $data = $formatted . ' ' . $symbol;
    return str_replace('  ', ' ', trim($data));
  }
}
...
if (!function_exists('vendorTotalAddedRoom')) {
  ...
  {
    if ($vendor_id != 0) {
      $total = Perahu::where('vendor_id', $vendor_id)->get()->count();
    } else {
      $total = 1;
    }
    return $total;
...
if (!function_exists('totalHotelRoom')) {
  ...
  {
    $totalReview = Perahu::where('hotel_id', $hotel_id)
      ->where('status', 1)
      ->count();
