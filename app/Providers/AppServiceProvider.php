      ...
      View::composer(['admin.*', 'vendors.*', 'frontend.*'], function ($view) use ($defaultLang, $settings, $langs) {
          ...
          if ($defaultLang && Schema::hasTable('page_contents')) {
              try {
                  ...
                  $allPages = DB::table('page_contents')->where('language_id', $defaultLang->id)->get();
                  $supportTitles = ['Pusat Bantuan', 'Kebijakan Privasi', 'Dukungan & Bantuan', 'Kebijakan Pembatalan', 'Ketentuan', 'Peta Situs'];
          ...
          // Dynamic Social Login Binding (FIX: Use Database Settings)
          if ($settings) {
              if (isset($settings->google_login_status) && $settings->google_login_status == 1) {
                  config([
                      'services.google.client_id' => $settings->google_client_id,
                      'services.google.client_secret' => $settings->google_client_secret,
                  ]);
              }
              if (isset($settings->facebook_login_status) && $settings->facebook_login_status == 1) {
                  config([
