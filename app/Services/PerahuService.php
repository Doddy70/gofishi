...
class PerahuService
{
    ...
    public function getAvailableRooms(array $filters, int $languageId, int $perPage = 12)
    {
        ...
        $query = Perahu::query()
            ->with([
                ...
                'hotel.hotel_contents' => function($q) use ($languageId) { $q->where('language_id', $languageId)->with('city'); },
                'vendor',
                'room_galleries'
            ])
    ...
    public function getRoomDetails(int $id, int $languageId)
    {
        return Perahu::with([
                ...
                'hotel.hotel_contents' => fn($q) => $q->where('language_id', $languageId)->with('city'),
                'vendor',
                'room_galleries',
                'packages'
            ])
