...
class VendorService
{
    ...
    public function register(array $data)
    {
        ...
        if (!$language) {
            throw new \RuntimeException('No language configured for vendor registration.');
        }
    ...
    protected function sendVerificationMail(array $data)
    {
        ...
        $url = url('vendor/email/verify?token=' . urlencode($data['verification_token']));
        $link = '<a href="' . $url . '" style="background-color: #FF385C; color: white; padding: 12px 24px; text-decoration: none; border-radius: 12px; font-weight: bold; display: inline-block;">Verifikasi Email Sekarang</a>';
        ...
        if ($info->smtp_status == 1) {
            ...
            // Expert implementation: Use Mailable for cleaner logic
            Mail::to($data['email'])->send(new \App\Mail\Vendor\VerifyEmail($data, $body, $mailTemplate->mail_subject));
