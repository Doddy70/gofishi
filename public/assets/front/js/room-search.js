...
$(document).ready(function () {
  ...
  $('#btnGetCurrentLocation').on('click', function() {
    if (navigator.geolocation) {
      $(this).html('<i class="fas fa-spinner fa-spin me-2"></i> Mencari Lokasi...');
      navigator.geolocation.getCurrentPosition(function(position) {
        $('#userLat').val(position.coords.latitude);
        $('#userLng').val(position.coords.longitude);
        $('#radiusFilter').show();
        $('#btnGetCurrentLocation').html('<i class="fal fa-map-marker-alt me-2"></i> Lokasi Aktif').removeClass('btn-outline-primary').addClass('btn-success');
        $('#page').val(1);
        updateUrl();
      }, function(error) {
        alert('Gagal mendapatkan lokasi: ' + error.message);
        $('#btnGetCurrentLocation').html('<i class="fal fa-map-marker-alt me-2"></i> Gunakan Lokasi Saat Ini');
      });
    } else {
  ...
  $('#radiusRange').on('input', function() {
    $('#radiusValText').text($(this).val());
  });

  $('#radiusRange').on('change', function() {
    $('#page').val(1);
    updateUrl();
  });
  ...
  $('#searchForm').on('submit', function (e) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        mapInitialize(mapId, featured_content, room_content);
      },
      error: function (xhr) {
        console.log(xhr);
      }
