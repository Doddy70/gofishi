...
@section('content')
  ...
  <div class="row">
              ...
              <div class="col-lg-12">
                ...
                <div class="form-group">
                  ...
                  <p class="mb-0 text-warning">
                    {{ __('To use Google Maps correctly, please generate a valid API Key from Google Cloud Platform. This key must be active and have billing enabled to ensure the Haversine radius search functionality displays accurate results.') }}
                  </p>
                  @if ($errors->has('google_maps_api_key'))
                    <p class="mt-1 mb-0 text-danger">{{ $errors->first('google_maps_api_key') }}</p>
                  @endif
                </div>
                ...
                <div class="form-group">
                  ...
                  @endif
                  <p class="mb-0 text-warning">
                    {{ __('After a location is seached, all the available lokasi / perahu which are located within this radius will be displayed.') }}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div class="col-lg-12">
                ...
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="whatsapp_api_url"
                    value="{{ $data->whatsapp_api_url ?? '' }}" placeholder="https://api.fonnte.com/send">
                  <p class="mt-2 mb-0 text-warning"><small>{{ __('The API URL for your WhatsApp Gateway provider (e.g., Fonnte, Wablas, dll). Leave empty to use legacy WhatsApp floating button only.') }}</small></p>

                  @if ($errors->has('whatsapp_api_url'))
                    <p class="mt-1 mb-0 text-danger">{{ $errors->first('whatsapp_api_url') }}</p>
                  @endif
                </div>

                <div class="form-group">
                  <label>{{ __('WhatsApp API Token') }}</label>
                  <input type="password" class="form-control" name="whatsapp_api_key"
                    value="{{ $data->whatsapp_api_key ?? '' }}" placeholder="Enter your token/api key">

                  @if ($errors->has('whatsapp_api_key'))
                    <p class="mt-1 mb-0 text-danger">{{ $errors->first('whatsapp_api_key') }}</p>
                  @endif
                </div>
              </div>
            </div>
    ...
    <div class="col-lg-4 col-md-6">
      <div class="card">
        <form action="{{ route('admin.settings.update_pixel') }}" method="post">
          @csrf
          <div class="card-header">
            <div class="row">
              <div class="col-lg-12">
                <div class="card-title">{{ __('Meta (Facebook) Pixel') }}</div>
              </div>
            </div>
          </div>
            ...
            <div class="row">
              <div class="col-lg-12">
                <div class="form-group">
                  ...
                  <div class="selectgroup w-100">
                    <label class="selectgroup-item">
                      <input type="radio" name="pixel_status" value="1" class="selectgroup-input"
                        {{ $data->pixel_status == 1 ? 'checked' : '' }}>
                      <span class="selectgroup-button">{{ __('Active') }}</span>
                    </label>

                    <label class="selectgroup-item">
                      <input type="radio" name="pixel_status" value="0" class="selectgroup-input"
                        {{ $data->pixel_status == 0 ? 'checked' : '' }}>
                      <span class="selectgroup-button">{{ __('Deactive') }}</span>
                    </label>
                  </div>
                  @if ($errors->has('pixel_status'))
                    <p class="mt-1 mb-0 text-danger">{{ $errors->first('pixel_status') }}</p>
                  @endif
                </div>
                ...
                <div class="form-group">
                  ...
                  @if ($errors->has('pixel_id'))
                    <p class="mt-1 mb-0 text-danger">{{ $errors->first('pixel_id') }}</p>
                  @endif
                </div>
              </div>
            </div>
    ...
    <div class="col-lg-4 col-md-6">
      <div class="card">
        <form action="{{ route('admin.settings.update_google_analytics') }}" method="post">
          @csrf
          <div class="card-header">
            <div class="row">
              <div class="col-lg-12">
            ...
            <div class="row">
              <div class="col-lg-12">
                <div class="form-group">
                  ...
                  <div class="selectgroup w-100">
                    <label class="selectgroup-item">
                      <input type="radio" name="google_analytics_status" value="1" class="selectgroup-input"
                        {{ $data->google_analytics_status == 1 ? 'checked' : '' }}>
                      <span class="selectgroup-button">{{ __('Active') }}</span>
                    </label>

                    <label class="selectgroup-item">
                      <input type="radio" name="google_analytics_status" value="0" class="selectgroup-input"
                        {{ $data->google_analytics_status == 0 ? 'checked' : '' }}>
                      <span class="selectgroup-button">{{ __('Deactive') }}</span>
                    </label>
                  </div>
                  @if ($errors->has('google_analytics_status'))
                    <p class="mt-1 mb-0 text-danger">{{ $errors->first('google_analytics_status') }}</p>
                  @endif
                </div>

                <div class="form-group">
                  <label>{{ __('Measurement ID (e.g., G-XXXX)') . '*' }}</label>
    ...
    <div class="col-lg-4 col-md-6">
      <div class="card">
        <form action="{{ route('admin.settings.update_google_gemini') }}" method="post">
          @csrf
          <div class="card-header">
            <div class="row">
              <div class="col-lg-12">
                <div class="card-title">{{ __('Google Gemini (AI Engine)') }}</div>
              </div>
            </div>
          </div>
            ...
            <div class="row">
              <div class="col-lg-12">
                <div class="form-group">
                  ...
                  <div class="selectgroup w-100">
                    <label class="selectgroup-item">
                      <input type="radio" name="google_gemini_status" value="1" class="selectgroup-input"
                        {{ $data->google_gemini_status == 1 ? 'checked' : '' }}>
                      <span class="selectgroup-button">{{ __('Active') }}</span>
                    </label>

                    <label class="selectgroup-item">
                      <input type="radio" name="google_gemini_status" value="0" class="selectgroup-input"
                        {{ $data->google_gemini_status == 0 ? 'checked' : '' }}>
                      <span class="selectgroup-button">{{ __('Deactive') }}</span>
                    </label>
                  </div>
                  @if ($errors->has('google_gemini_status'))
                    <p class="mt-1 mb-0 text-danger">{{ $errors->first('google_gemini_status') }}</p>
                  @endif
                </div>

                <div class="form-group">
                  <label>{{ __('Gemini API Key') . '*' }}</label>
                  <input type="password" class="form-control" name="google_gemini_api_key"
                    value="{{ $data->google_gemini_api_key }}">
                  @if ($errors->has('google_gemini_api_key'))
                    <p class="mt-1 mb-0 text-danger">{{ $errors->first('google_gemini_api_key') }}</p>
                  @endif
                  <p class="text-warning mt-2 mb-0">
                    <small>{{ __('Required. Get your API key from Google AI Studio.') }}</small>
                  </p>
                </div>

                <div class="form-group">
                  <label>{{ __('AI Engine System Prompt') }}</label>
                  <textarea class="form-control" name="google_gemini_prompt" rows="5">{{ $data->google_gemini_prompt }}</textarea>
                  <p class="text-warning mt-2 mb-0">
                    <small>{{ __('Default system behavior for Smart Search and AI Chat.') }}</small>
                  </p>
                </div>
              </div>
            </div>
