...
@section('content')
  <div class="page-header">
    ...
    <ul class="breadcrumbs">
      ...
      <li class="nav-item">
        <a href="#">{{ __('Lokasi Management') }}</a>
      </li>
      <li class="separator">
        <i class="flaticon-right-arrow"></i>
      </li>
      <li class="nav-item">
        <a href="#">{{ __('Featured Lokasi') }}</a>
      </li>
      <li class="separator">
        <i class="flaticon-right-arrow"></i>
      </li>
    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header">
          <div class="row">
            <div class="col-lg-10">
              <form id="searchForm" action="{{ route('admin.lokasi_management.featured_lokasi.rejected_request') }}"
                method="GET">
                <div class="row">
                  <div class="col-lg-3">
            ...
            <div class="col-lg-2">
              <button class="btn btn-danger btn-sm d-none bulk-delete float-lg-right"
                data-href="{{ route('admin.lokasi_management.featured_lokasi.bulk_delete_order') }}"
                  ...
                  <table class="table table-striped mt-2">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">{{ __('Lokasi Title') }}</th>
                    ...
                    <tbody>
                      @foreach ($orders as $order)
                        <tr>
                          ...
                          <td class="title">
                            ...
                            @if (!empty($hotel_content))
                              <a href="{{ route('frontend.lokasi.details', ['slug' => $hotel_content->slug, 'id' => $hotel_content->hotel_id]) }}"
                                target="_blank">
                            ...
                            @else
                              @if ($order->payment_status == 'pending')
                                <form id="paymentStatusForm-{{ $order->id }}" class="d-inline-block"
                                  action="{{ route('admin.lokasi_management.featured_lokasi.update_payment_status', ['id' => $order->id]) }}"
                                  method="post">
                                  @csrf
                                  <select
                          ...
                          <td>
                            @if ($order->order_status == 'pending')
                              <form id="orderStatusForm-{{ $order->id }}" class="d-inline-block"
                                action="{{ route('admin.lokasi_management.featured_lokasi.update_order_status', ['id' => $order->id]) }}"
                                method="post">
                                @csrf
                                <select
                            ...
                            <div class="dropdown">
                              ...
                              <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                ...
                                <form class="deleteForm d-block"
                                  action="{{ route('admin.lokasi_management.featured_lokasi.delete', ['id' => $order->id]) }}"
                                  method="post">
                                  @csrf
                        ...
                        @includeIf('admin.lokasi-management.featured-hotel.show-receipt')
                        @includeIf('admin.lokasi-management.featured-hotel.details')
                      @endforeach
                    </tbody>
                  </table>
                </div>
