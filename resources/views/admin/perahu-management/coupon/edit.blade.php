  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="ajaxEditForm" class="modal-form" action="{{ route('admin.perahu_management.coupon.update') }}"
          method="post">
          @csrf
          ...
          <div class="row no-gutters">
            ...
            <div class="col-lg-12"> 
              <div class="form-group">
                <label for="">{{ __('Perahu') }}</label>
                <select name="rooms[]" class="form-control select2" multiple="multiple" id="in_rooms">
                  @foreach (($rooms ?? collect()) as $roomItem)
                    <option value="{{ $roomItem->id }}">
                      {{ $roomItem->title }} ({{ $roomItem->vendor->username ?? 'Admin' }})
                    </option>
                  @endforeach
                </select>
                <p class="text-warning mt-2 mb-0">
                  <small>
                    {{ __('This coupon can be applied to these perahu.') }}<br>
                    {{ __('Leave this field empty for all perahu.') }}
                  </small>
                </p>
              </div>
            </div>
