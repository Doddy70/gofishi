...
@section('content')
  <!-- Hero section start-->
  <section class="hero-banner hero-banner_v2 header-next" style="position: relative; z-index: 1;">
    <div class="container-fluid">
      ...
      <div class="banner-filter-form mt-40 mx-auto">
        <div class="form-wrapper py-20 px-30 border">
          <form action="{{ route('frontend.perahu') }}" id="searchForm2" method="GET">
            <div class="grid">
              <div class="item">
                <div class="form-group">
                  <label for="place">{{ __('Lokasi') }}</label>
                  <div class="form-block">
                    <div class="icon">
                      <i class="far fa-map-marker-alt"></i>
                    </div>
                    <input type="text" name="location" class="form-control" placeholder="{{ __('Kota, tujuan, atau nama kapal') }}"
                      id="search-address">
              ...
              <div class="item">
                <div class="form-group">
                  <label for="guests">{{ __('Jumlah Tamu') }}</label>
                  <div class="form-block">
                    <div class="icon">
                      <i class="fas fa-user-friends"></i>
                    </div>
                    <div class="d-flex w-100 align-items-center column-gap-2">
                        <input type="number" class="form-control bg-transparent border-0 px-2" name="adult" placeholder="{{ __('Dewasa') }}" min="1" value="1" title="{{ __('Dewasa') }}" style="width: 50%" />
                        <input type="number" class="form-control bg-transparent border-0 px-2" name="children" placeholder="{{ __('Anak') }}" min="0" value="0" title="{{ __('Anak') }}" style="width: 50%" />
                    </div>
                  </div>
                </div>
              </div>
                ...
                @foreach ($cities as $city)
                  <div class="swiper-slide">
                    <div class="card mb-30 rounded-pill">
                      <div class="card_img">
                        <a href="{{ route('frontend.lokasi', ['city' => $city->id]) }}" title="{{ $city->name }}"
                      ...
                      <div class="card_content">
                        <h4 class="color-white mb-30 lc-1">
                          <a href="{{ route('frontend.lokasi', ['city' => $city->id]) }}" target="_self"
                            title="{{ __('Link') }}">{{ $city->name }}</a>
                        </h4>
                        <a href="{{ route('frontend.lokasi', ['city' => $city->id]) }}"
          ...
          <div class="col-lg-6">
            <div class="content-title fluid-left pb-20">
              <h2 class="title mb-20">
                {{ $sectionContent ? $sectionContent->featured_section_title : __('Your Ultimate Hourly Lokasi Booking Solution') }}
              </h2>
              <p>
        ...
        <div class="row">
          <div class="col-12" data-aos="fade-up">
            <div class="section-title title-inline mb-50">
              <div>
                <h2 class="title">
                  {{ !empty($sectionContent->featured_room_section_title) ? $sectionContent->featured_room_section_title : __('Popular Lokasi Perahu') }}
                </h2>
              </div>
              @if (count($room_contents_count) > count($room_contents))
                <a href="{{ route('frontend.perahu') }}" class="btn btn-lg btn-primary rounded-pill"
                  title="{{ !empty($sectionContent->featured_room_section_button_text) ? $sectionContent->featured_room_section_button_text : __('View All Perahu') }}"
                  target="_self">{{ !empty($sectionContent->featured_room_section_button_text) ? $sectionContent->featured_room_section_button_text : __('View All Perahu') }}</a>
              @endif
            </div>
          </div>
          <div class="col-12" data-aos="fade-up">
            <div class="row">
              @if (count($room_contents) < 1)
                <div class="p-3 text-center bg-light radius-md">
                  <h6 class="mb-0">{{ __('NO FEATURED PERAHU FOUND') }}</h6>
                </div>
              @else
                  ...
                  <div class="col-xl-4 col-md-6">
                    <div class="product-default border radius-md mb-25">
                      <figure class="product_img">
                        <a href="{{ route('frontend.perahu.details', ['slug' => $room->slug, 'id' => $room->id]) }}"
                          target="_self" title="{{ __('Link') }}" class="lazy-container ratio ratio-2-3 radius-sm">
                          <img class="lazyload"
                            src="{{ asset('assets/img/perahu/featureImage/' . $room->feature_image) }}" alt="Perahu">
                        </a>
                        ...
                        <a href="{{ $checkWishList == false ? route('frontend.perahu.add_wishlist', $room->id) : route('frontend.perahu.remove_wishlist', $room->id) }}"
                        ...
                        <div class="p-20">
                          ...
                          <div class="product_author mt-15">
                            <a class="d-flex align-items-center gap-1"
                              href="{{ route('frontend.lokasi.details', ['slug' => $room->hotelSlug, 'id' => $room->hotelId]) }}"
                      ...
                      <div class="product_bottom p-20 border-top text-center"></div>
                      </div>
                    </div>
                    <!-- product-default -->
                  </div>
      ...
      <div class="container">
        <div class="section-title title-inline mb-50" data-aos="fade-up">
          ...
          @if (count($blog_count) > count($blogs))
            <a href="{{ route('frontend.lokasi') }}" class="btn btn-lg btn-primary radius-sm"
            ...
            @foreach ($blogs as $blog)
              <div class="col-md-6 col-xl-4">
                <article class="card border radius-md mb-30">
                  <div class="card_img">
                    <a href="{{ route('frontend.lokasi.details', ['slug' => $blog->slug, 'id' => $blog->id]) }}" target="_self"
                  ...
                  <div class="card_content p-20">
                    ...
                    <div class="cta-btn mt-20">
                      <a href="{{ route('frontend.lokasi.details', ['slug' => $blog->slug, 'id' => $blog->id]) }}"
