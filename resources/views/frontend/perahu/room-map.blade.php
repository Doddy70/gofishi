...
@section('content')
  ...
  <!-- Page title start-->
  <div class="listing-map header-next2 d-none d-lg-block">
    <div class="container">
      <div class="main-map">
        <div id="main-map"></div>
      </div>
        ...
        <div class="col-xl-3" data-aos="fade-up">
          <div class="widget-offcanvas offcanvas-xl offcanvas-start sticky" tabindex="-1" id="widgetOffcanvas"
            ...
            <div class="sidebar-scroll">
              ...
              @include('frontend.perahu.side-bar')
            </div>
          </div>
          <!-- Spacer -->
          ...
          <div class="search-container mb-40">
            @if (count($featured_contents) < 1 && count($room_contents) < 1)
              <div class="p-3 text-center bg-light radius-md">
                <h6 class="mb-0">{{ __('NO PERAHU FOUND') }}</h6>
              </div>
            @else
                    ...
                    <div class="product-default product-default-style-2 border radius-md mb-25  border-primary featured">
                      <figure class="product_img">
                        <a href="{{ route('frontend.perahu.details', ['slug' => $room->slug, 'id' => $room->id]) }}"
                          ...
                          <img class="lazyload" src="{{ asset('assets/img/perahu/featureImage/' . $room->feature_image) }}"
                            alt="{{ __('Perahu Image') }}">
                        </a>
                      ...
                      <div class="product_details">
                        ...
                        <!-- Harga per jam disembunyikan sesuai kebutuhan -->
                      </div>
                    </div>
                    <!-- product-default -->
                  </div>
                    ...
                    <div class="product-default product-default-style-2 border radius-md mb-25">
                      <figure class="product_img">
                        <a href="{{ route('frontend.perahu.details', ['slug' => $room->slug, 'id' => $room->id]) }}"
                          target="_self" title="{{ __('Link') }}" class="lazy-container ratio ratio-2-3 radius-sm">
                          <img class="lazyload"
                            src="{{ asset('assets/img/perahu/featureImage/' . $room->feature_image) }}"
                            alt="{{ __('Perahu Image') }}">
                        </a>
                        @if (Auth::guard('web')->check())
                          @php
  ...
  <form action="{{ route('frontend.perahu') }}" id="searchForm" method="GET">
...
@section('script')
  ...
  <script>
    ...
    var searchUrl = "{{ route('frontend.perahu') }}";
