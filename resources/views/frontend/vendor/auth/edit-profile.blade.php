          ...
          <div class="row">
            <div class="col-lg-8 mx-auto">
              ...
              <form id="commonForm" action="{{ route('vendor.update_profile') }}" method="post">
                ...
                <h2 class="mt-5">{{ __('Public Information') }}</h2>
                <hr>
                <div class="row">
                  <div class="col-lg-12">
                    <div class="nav-tabs-custom">
                      <ul class="nav nav-tabs border-bottom-0">
                        @foreach ($languages as $lang)
                          <li class="nav-item">
                            <a class="nav-link {{ $loop->first ? 'active' : '' }}" data-toggle="tab" href="#lang_{{ $lang->code }}">
                              {{ $lang->name }}
                            </a>
                          </li>
                        @endforeach
                      ...
                      <div class="tab-content mt-4">
                        @foreach ($languages as $lang)
                        @php
                            $vendorInfo = $vendor->vendor_infos()->where('language_id', $lang->id)->first();
                        @endphp
                        <div id="lang_{{ $lang->code }}" class="tab-pane {{ $loop->first ? 'active' : '' }}">
                          <div class="row">
                            <div class="col-lg-6">
                              <div class="form-group">
                                <label>{{ __('Name') . ' (' . $lang->name . ')*' }}</label>
                                <input type="text" value="{{ $vendorInfo ? $vendorInfo->name : '' }}" class="form-control" name="{{ $lang->code }}_name">
                              </div>
                            </div>
                            <div class="col-lg-6">
                              <div class="form-group">
                                <label>{{ __('Pekerjaan') . ' (' . $lang->name . ')' }}</label>
                                <input type="text" value="{{ $vendorInfo ? $vendorInfo->occupation : '' }}" class="form-control" name="{{ $lang->code }}_occupation" placeholder="Contoh: Kapten Kapal Tradisional">
                              </div>
                            </div>
                            <div class="col-lg-12">
                              <div class="form-group">
                                <label>{{ __('Bahasa yang Dikuasai') . ' (' . $lang->name . ')' }}</label>
                                <input type="text" value="{{ $vendorInfo ? $vendorInfo->languages : '' }}" class="form-control" name="{{ $lang->code }}_languages" placeholder="Contoh: Bahasa Indonesia, English">
                                <small class="text-muted">{{ __('Pisahkan dengan koma') }}</small>
                              </div>
                            </div>
                            <div class="col-lg-12">
                              <div class="form-group">
                                <label>{{ __('Tentang Saya / Bio') . ' (' . $lang->name . ')' }}</label>
                                <textarea name="{{ $lang->code }}_details" class="form-control" rows="4" placeholder="Ceritakan sedikit tentang diri Anda...">{{ $vendorInfo ? $vendorInfo->details : '' }}</textarea>
                              </div>
                            </div>
                            {{-- Hidden fields to maintain compatibility with existing logic --}}
                            <input type="hidden" name="{{ $lang->code }}_country" value="{{ $vendorInfo ? $vendorInfo->country : '' }}">
                            <input type="hidden" name="{{ $lang->code }}_city" value="{{ $vendorInfo ? $vendorInfo->city : '' }}">
                            <input type="hidden" name="{{ $lang->code }}_state" value="{{ $vendorInfo ? $vendorInfo->state : '' }}">
                            <input type="hidden" name="{{ $lang->code }}_zip_code" value="{{ $vendorInfo ? $vendorInfo->zip_code : '' }}">
                            <input type="hidden" name="{{ $lang->code }}_address" value="{{ $vendorInfo ? $vendorInfo->address : '' }}">
                          </div>
                        </div>
                        @endforeach
                      </div>
