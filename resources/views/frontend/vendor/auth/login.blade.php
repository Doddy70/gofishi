    ...
    <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-10">
      ...
      @if (!empty($bs) && ($bs->google_login_status == 1 || $bs->facebook_login_status == 1))
        <div class="space-y-4 mb-8">
          <p class="text-center text-sm text-gray-500">{{ __('Atau masuk dengan akun sosial Anda') }}</p>
          <div class="grid grid-cols-1 gap-3">
            @if ($bs->google_login_status == 1)
              <a href="{{ route('vendor.login.provider', ['provider' => 'google']) }}" class="w-full inline-flex items-center justify-center gap-3 rounded-xl border border-gray-200 px-4 py-3 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">
            ...
            @if ($bs->facebook_login_status == 1)
              <a href="{{ route('vendor.login.provider', ['provider' => 'facebook']) }}" class="w-full inline-flex items-center justify-center gap-3 rounded-xl border border-gray-200 px-4 py-3 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">
                <img src="https://www.svgrepo.com/show/452223/facebook-color.svg" alt="Facebook" class="h-5 w-5" />
                {{ __('Login dengan Facebook') }}
              </a>
            @endif
          </div>
        </div>
