          ...
          <ul class="pricing-content">
            <li>
              ...
              @elseif($package->number_of_hotel == 1)
                {{ __('Lokasi') }} ({{ $package->number_of_hotel }})
              @else
                {{ __('Lokasi') }}({{ $package->number_of_hotel }})
              @endif
            </li>
            ...
            <li>
              ...
              @elseif($package->number_of_room == 1)
                {{ __('Perahu') }} ({{ $package->number_of_room }})
              @else
                {{ __('Perahu') }}({{ $package->number_of_room }})
              @endif
            </li>
            ...
            <li>
              @if ($package->number_of_amenities_per_hotel == 999999)
                {{ __('Amenitie Per Lokasi (Unlimited)') }}
              @elseif($package->number_of_amenities_per_hotel == 1)
                {{ __('Amenitie Per Lokasi') }} ({{ $package->number_of_amenities_per_hotel }})
              @else
                {{ __('Amenities Per Lokasi') }}({{ $package->number_of_amenities_per_hotel }})
              @endif
            </li>

            <li>
              @if ($package->number_of_amenities_per_room == 999999)
                {{ __('Amenitie Per Perahu (Unlimited)') }}
              @elseif($package->number_of_amenities_per_room == 1)
                {{ __('Amenitie Per Perahu') }} ({{ $package->number_of_amenities_per_room }})
              @else
                {{ __('Amenities Per Perahu') }}({{ $package->number_of_amenities_per_room }})
              @endif
            </li>

            <li>
              @if ($package->number_of_images_per_hotel == 999999)
                {{ __('Image Per Lokasi (Unlimited)') }}
              @elseif($package->number_of_images_per_hotel == 1)
                {{ __('Image Per Lokasi') }} ({{ $package->number_of_images_per_hotel }})
              @else
                {{ __('Images Per Lokasi') }}({{ $package->number_of_images_per_hotel }})
              @endif
            </li>

            <li>
              @if ($package->number_of_images_per_room == 999999)
                {{ __('Image Per Perahu (Unlimited)') }}
              @elseif($package->number_of_images_per_room == 1)
                {{ __('Image Per Perahu') }} ({{ $package->number_of_images_per_room }})
              @else
                {{ __('Images Per Perahu') }}({{ $package->number_of_images_per_room }})
              @endif
            </li>
            <li>
