...
@section('content')
  ...
  <div class="mt-2 mb-4">
    @if(Auth::guard('vendor')->user()->document_verified == 0)
      <div class="alert alert-warning text-dark">
        <i class="fas fa-file-invoice"></i> <strong>{{ __('Verification Status') }}: {{ __('Pending') }}</strong>. 
        {{ __('Your documents are currently being reviewed by our team. You will be notified via email once verified.') }}
      </div>
    @elseif(Auth::guard('vendor')->user()->document_verified == 2)
      <div class="alert alert-danger text-dark">
        <i class="fas fa-exclamation-triangle"></i> <strong>{{ __('Verification Status') }}: {{ __('Rejected') }}</strong>.
        @if(Auth::guard('vendor')->user()->rejection_reason)
          <br><strong>{{ __('Reason') }}:</strong> {{ Auth::guard('vendor')->user()->rejection_reason }}
        @endif
        <br>{{ __('Please update your documents in the profile section to resubmit for verification.') }}
        <a href="{{ route('vendor.edit.profile') }}" class="btn btn-sm btn-primary ml-2">{{ __('Update Documents') }}</a>
      </div>
    @elseif(Auth::guard('vendor')->user()->document_verified == 1)
      <div class="alert alert-success text-dark">
        <i class="fas fa-check-circle"></i> <strong>{{ __('Verification Status') }}: {{ __('Verified') }}</strong>.
        {{ __('Your documents have been approved. You have full access to manage your perahu and locations.') }}
      </div>
    @endif
  </div>
    ...
    <div class="col-lg-6">
      <div class="card">
        <div class="card-header">
          <div class="card-title">{{ __('Monthly Perahu Posts') }} ({{ date('Y') }})</div>
        </div>
