...
<head>
  ...
  <link rel="stylesheet" href="{{ asset('assets/admin/css/vendor-modern.css') }}">
...
<body data-background-color="{{ Auth::guard('vendor')->user()->vendor_theme_version == 'light' ? 'white2' : 'dark' }}"
  ...
  {{-- Airbnb-Style Unverified Identity Notification Modal --}}
  @if(Auth::guard('vendor')->check() && Auth::guard('vendor')->user()->document_verified != 1)
      @include('vendors.partials.verification_modal')
  @endif
