...
@section('content')
  <div class="page-header">
    ...
    <a href="{{ route('vendor.perahu_bookings.all_bookings', ['language' => $defaultLang->code]) }}"
      ...
      <div class="card">
        <div class="card-header">
          <div class="card-title d-inline-block">
            {{ __('Booking Status & Metadata') }}
          </div>
        </div>
        <div class="card-body">
            <div class="payment-information">
                <div class="row mb-2">
                    <div class="col-lg-6"><strong>{{ __('Status Pesanan') }} :</strong></div>
                    <div class="col-lg-6">
                        <span class="badge badge-{{ $details->order_status == 'pending' ? 'warning' : ($details->order_status == 'approved' ? 'success' : 'danger') }}">
                            {{ ucfirst(__($details->order_status)) }}
                        </span>
                    </div>
                </div>
                ...
                <div class="row mb-2">
                    <div class="col-lg-6 text-danger"><strong>{{ __('Alasan Penolakan') }} :</strong></div>
                    <div class="col-lg-6 text-danger small">{{ $details->rejection_reason }}</div>
                </div>
                @endif

                <div class="row mb-2 border-top pt-2 mt-2">
                    <div class="col-lg-6"><strong>{{ __('Nama KM') }} :</strong></div>
                    <div class="col-lg-6 text-primary fw-bold">{{ $details->hotelRoom->nama_km ?? '--' }}</div>
                </div>
                <div class="row mb-2">
                    <div class="col-lg-6"><strong>{{ __('Kapten') }} :</strong></div>
                    <div class="col-lg-6">{{ $details->hotelRoom->nama_kapten ?? '--' }}</div>
                </div>
                <div class="row mb-2 border-top pt-2 mt-2">
                    <div class="col-lg-6"><strong>{{ __('Verifikasi Usia') }} :</strong></div>
                    <div class="col-lg-6">
                        @if($details->age_confirmed)
                            <span class="text-success"><i class="fas fa-check-circle mr-1"></i> {{ __('Sudah Verifikasi 17+') }}</span>
                        @else
                            <span class="text-danger"><i class="fas fa-times-circle mr-1"></i> {{ __('Belum Verifikasi') }}</span>
                        @endif
                    </div>
                </div>
            </div>
            ...
            @if (!is_null($details->received_amount))
              <div class="row mb-2">
                <div class="col-lg-6">
                  ...
                  Rp {{ number_format($details->received_amount, 2, ',', '.') }}
              </div>
            @endif
            ...
            @if (!is_null($details->received_amount))
              <div class="row mb-2">
                <div class="col-lg-6">
                  ...
                  Rp {{ number_format($details->comission, 2, ',', '.') }}
      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="payment-information">
            <div class="row mb-2">
              <div class="col-lg-4">
                <strong>{{ __('Perahu') . ' :' }}</strong>
              </div>
              <div class="col-lg-8">
                @if ($roomContentInfo != '--')
                  <a target="_blank"
                    href="{{ route('frontend.perahu.details', ['id' => $roomContentInfo->room_id, 'slug' => $roomContentInfo->slug]) }}">{{ @$roomContentInfo->title }}</a>
                @else
                  {{ $roomContentInfo }}
                @endif
              </div>
            ...
            <div class="row mb-2">
              ...
              <div class="col-lg-8">
                @if ($hotelContentInfo != '--')
                  <a target="_blank"
                    href="{{ route('frontend.lokasi.details', ['id' => $hotelContentInfo->hotel_id, 'slug' => $hotelContentInfo->slug]) }}">{{ @$hotelContentInfo->title }}</a>
                @else
                  {{ $hotelContentInfo }}
                @endif
              </div>
  ...
  @include('vendors.perahu-booking.services')
@endsection
