                            ...
                            <div class="col-lg-12">
                              <div class="form-group {{ $language->direction == 1 ? 'rtl text-right' : '' }}">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    ...
                                    @endphp
                                    @if($aiActive)
                                        <button type="button" class="btn btn-primary btn-xs ai-gen-btn" 
                                                data-lang="{{ $language->code }}"
                                                onclick="generateAiDescription('{{ $language->code }}')">
                                            <i class="fas fa-magic mr-1"></i> {{ __('✨ Buat Deskripsi AI') }}
                                        </button>
                                    @endif
                                </div>
...
@section('variables')
  ...
  <script>
    function generateAiDescription(lang) {
        const btn = $(`.ai-gen-btn[data-lang="${lang}"]`);
        const originalText = btn.html();
        
        // Collect specs
        const specs = {
            title: $('input[name="nama_km"]').val(),
            length: $('input[name="boat_length"]').val(),
            width: $('input[name="boat_width"]').val(),
            capacity: $('input[name="adult"]').val(),
            engine: $('input[name="engine_1"]').val() + ($('input[name="engine_2"]').val() ? ' & ' + $('input[name="engine_2"]').val() : ''),
            amenities: []
        };

        // Collect checked amenities
        $(`input[name="${lang}_amenities[]"]:checked`).each(function() {
            specs.amenities.push($(this).next('label').text().trim());
        });

        if (!specs.title) {
            alert('Mohon isi Nama Kapal terlebih dahulu agar AI bisa bekerja.');
            return;
        }

        btn.html('<i class="fas fa-spinner fa-spin mr-1"></i> Menyusun...').prop('disabled', true);

        $.ajax({
            url: "{{ route('vendor.smart_ai.generate_description') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                title: specs.title,
            ...
            success: function(response) {
                if (response.status === 'success') {
                    $(`#${lang}_description`).summernote('code', response.description);
                } else {
                    alert('Gagal membuat deskripsi: ' + response.message);
                }
            },
            error: function() {
